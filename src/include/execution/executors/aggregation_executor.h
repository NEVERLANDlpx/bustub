...
class SimpleAggregationHashTable {
 public:
  ...
  void CombineAggregateValues(AggregateValue *result, const AggregateValue &input) {
    for (uint32_t i = 0; i < agg_exprs_.size(); i++) {  //循环遍历所有的聚合表达式
      switch (agg_types_[i]) {
        case AggregationType::CountStarAggregate:
...
class AggregationExecutor : public AbstractExecutor {
 ...
 private:
  ...
  // TODO(Student): Uncomment SimpleAggregationHashTable::Iterator aht_iterator_; simpleaggregATionhashtable;;iterator aht-iterator-;
